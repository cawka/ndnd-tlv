/**
 * @file ndndsmoketest.c
 * Simple program for smoke-test of ndnd.
 *
...
open_local(struct sockaddr_un *sa, const char *verb)
{
    ...
    if (res == -1 && errno == ENOENT) {
        ...
        if (verb != NULL && 0 == strcmp(verb, "kill"))
            exit(1);
        /* Retry after a delay in case ndnd was just starting up. */
        sleep(1);
        res = connect(sock, (struct sockaddr *)sa, sizeof(*sa));
    }
    if (res == -1) {
...
static void
send_ndnb_file(int sock, FILE *msgs, const char *filename, int is_dgram)
{
    ssize_t rawlen;
    ssize_t sres;
    int fd = 0;
...
static int
is_ndnb_name(const char *s)
{
    size_t len = strlen(s);
    return (len > 5 && 0 == strcasecmp(s + len - 5, ".ndnb"));
}
...
main(int argc, char **argv)
{
    ...
    ndn_setup_sockaddr_un(portstr, &addr);
    if (udp)
    ...
    for (argp = optind; argv[argp] != NULL; argp++) {
        if (0 == strcmp(argv[argp], "send")) {
            ...
            if (filename == NULL)
                ...
                argp++;
            send_ndnb_file(sock, msgs, filename, udp);
        }
        else if (is_ndnb_name(argv[argp])) {
            filename = argv[argp];
            send_ndnb_file(sock, msgs, filename, udp);
        }
