 ...
 */
struct ndn_bloom *
ndn_bloom_create(int estimated_members, const unsigned char seed[4])
{
    struct ndn_bloom *ans = NULL;
    struct ndn_bloom_wire *f;
    ...
    if (f != NULL) {
        ...
        ndn_bloom_destroy(&ans);
    return(ans);
}

const struct ndn_bloom_wire *
ndn_bloom_validate_wire(const void *buf, size_t size)
{
    const struct ndn_bloom_wire *f = (const struct ndn_bloom_wire *)buf;
    if (size < 9)
        return (NULL);
...
struct ndn_bloom *
ndn_bloom_from_wire(const void *data, size_t size)
{
    struct ndn_bloom *ans = NULL;
    const struct ndn_bloom_wire *f = ndn_bloom_validate_wire(data, size);
    if (f != NULL) {
        ans = calloc(1, sizeof(*ans));
        if (ans == NULL) return(ans);
...
void
ndn_bloom_destroy(struct ndn_bloom **bp)
{
    if (*bp != NULL) {
        if ((*bp)->wire != NULL)
            free((*bp)->wire);
...
static int
bloom_seed(const struct ndn_bloom_wire *f)
{
    unsigned u;
 ...
 */
int
ndn_bloom_insert(struct ndn_bloom *b, const void *key, size_t size)
{
    
    int d = 0;
    struct ndn_bloom_wire *f = b->wire;
    int h, i, k, m, n, s;
...
int
ndn_bloom_match_wire(const struct ndn_bloom_wire *f, const void *key, size_t size)
{
    int h, i, k, m, n, s;
    const unsigned char *hb = (const unsigned char *)key;
    n = f->n_hash;
...
int
ndn_bloom_match(struct ndn_bloom *b, const void *key, size_t size)
{
    return(ndn_bloom_match_wire(b->wire, key, size));
}

int
ndn_bloom_n(struct ndn_bloom *b)
{
    return(b->n);
}

int
ndn_bloom_wiresize(struct ndn_bloom *b)
{
    // XXX - in principle, this could fold the filter if it is excessively large
    const struct ndn_bloom_wire *f = (b->wire);
    if (f == NULL)
        return(-1);
...
int
ndn_bloom_store_wire(struct ndn_bloom *b, unsigned char *dest, size_t destsize)
{
    // XXX - in principle, this could fold the filter if it is excessively large
    int wiresize = ndn_bloom_wiresize(b);
