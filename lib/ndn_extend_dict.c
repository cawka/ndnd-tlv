...
{
    const struct ndn_dict_entry *ex = x;
    const struct ndn_dict_entry *ey = y;
...
/**
 * Destroy a dictionary dynamically allocated by ndn_extend_dict
 * @param dp    Pointer to a pointer to a ndn_dict which will be freed
 *              and set to NULL
 */
void
ndn_destroy_dict(struct ndn_dict **dp)
{
    struct ndn_dict *d = *dp;
    int i;
    if (d != NULL) {
        for (i = 0; i < d->count; i++) {
 ...
 */
int
ndn_extend_dict(const char *dict_file, struct ndn_dict *d, struct ndn_dict **rdp)
{
    FILE *df = NULL;
    int i, c;
    struct ndn_dict_entry *ndd = NULL;
    struct ndn_dict_entry *ndd_tmp = NULL;
    int ndc = 0;
    struct ndn_charbuf *enamebuf = NULL;
    unsigned int eindex = 0;;
    struct ndn_dict *nd = NULL;
    enum scanner_state {
        S_OVERFLOW = -2,
        S_ERROR = -1,
        S_INITIAL = 0,
    ...
    while ((c = fgetc(df)) != EOF && s >= S_INITIAL) {
        switch (s) {
            ...
            case S_NAME:
                if (isalnum(c)) {
                    ndn_charbuf_append_value(enamebuf, c, 1);
                } else if (c == ',' || c == '\n') {
                    /* construct entry */
                    ...
                    ndd[ndc].name = strdup(ndn_charbuf_as_string(enamebuf));
                    ndc++;
                    ndn_charbuf_reset(enamebuf);
