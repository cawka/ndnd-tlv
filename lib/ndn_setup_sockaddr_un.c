 ...
 *
 * If the environment variable NDN_LOCAL_SOCKNAME is set and
 * not empty, it supplies the name stem; otherwise the compiled-in
 * default is used.
 *
 * If portstr is NULL or empty, the environment variable NDN_LOCAL_PORT is
 * checked. If the portstr specifies something other than the ndnx registered
 ...
 */
void
ndn_setup_sockaddr_un(const char *portstr, struct sockaddr_un *result)
{
    struct sockaddr_un *sa = result;
    const char *sockname = getenv("NDN_LOCAL_SOCKNAME");
    if (sockname == NULL || sockname[0] == 0)
        sockname = NDN_DEFAULT_LOCAL_SOCKNAME; /* /tmp/.ndnd.sock */
    memset(sa, 0, sizeof(*sa));
    sa->sun_family = AF_UNIX;
    if (portstr == NULL || portstr[0] == 0)
        portstr = getenv(NDN_LOCAL_PORT_ENVNAME);
    if (portstr != NULL && atoi(portstr) > 0 &&
          atoi(portstr) != atoi(NDN_DEFAULT_UNICAST_PORT))
...
/**
 * Set up a Internet socket address for contacting ndnd.
 *
