...
struct ndn_sigc *
ndn_sigc_create(void)
{
    return (calloc(1, sizeof(struct ndn_sigc)));
}

void
ndn_sigc_destroy(struct ndn_sigc **ctx)
{
    if (*ctx) {
...
int
ndn_sigc_init(struct ndn_sigc *ctx, const char *digest, const struct ndn_pkey *priv_key)
{
    const EVP_MD *md;
...
int
ndn_sigc_update(struct ndn_sigc *ctx, const void *data, size_t size)
{
...
int
ndn_sigc_final(struct ndn_sigc *ctx, struct ndn_signature *signature, size_t *size, const struct ndn_pkey *priv_key)
{
...
size_t
ndn_sigc_signature_max_size(struct ndn_sigc *ctx, const struct ndn_pkey *priv_key)
{
...
struct ndn_pkey *
ndn_d2i_pubkey(const unsigned char *p, size_t size)
{
    const unsigned char *q = p;
    EVP_PKEY *ans;
    ans = d2i_PUBKEY(NULL, &q, size);
    return ((struct ndn_pkey *)ans);
}

void
ndn_pubkey_free(struct ndn_pkey *i_pubkey)
{
...
size_t
ndn_pubkey_size(const struct ndn_pkey *i_pubkey)
{
    size_t ans;
    EVP_PKEY *pkey = (EVP_PKEY *)i_pubkey;
 ...
 */
void
ndn_random_bytes(unsigned char *buf, size_t size)
{
    int num = size;
