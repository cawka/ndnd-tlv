 ...
 */
int
ndn_charbuf_append_sockaddr(struct ndn_charbuf *c, const struct sockaddr *sa)
{
    ...
    s = inet_ntop(sa->sa_family, rawaddr, 
                  (void *)ndn_charbuf_reserve(c, sz), sz);
    if (s == NULL) {
        c->length = savlen;
        return(-1);
    }
    c->length += strlen(s);
    ndn_charbuf_append_string(c, closer);
    return(port);
}
