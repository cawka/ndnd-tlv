...
namespace error {
struct Tlv : public std::runtime_error { Tlv(const std::string &what) : std::runtime_error(what) {} };
} // error
 ...
 *
 * This call will throw ndn::error::Tlv (aka std::runtime_error) if number cannot be read
 *
...
{
  if (begin == end)
    throw new error::Tlv("Empty buffer during TLV processing");
  ...
  if (value < 253)
    ...
    {
      if (end - begin < 2)
        throw new error::Tlv("Insufficient data during TLV processing");
...
readType(InputIterator &begin, const InputIterator &end)
{
  ...
  if (type > std::numeric_limits<uint32_t>::max())
    {
      throw new error::Tlv("TLV type code exceeds allowed maximum");
    }
...
readNonNegativeInteger(size_t size, InputIterator &begin, const InputIterator &end)
{
  ...
  throw new error::Tlv("Invalid length for nonNegativeInteger (only 1, 2, 4, and 8 are allowed)");
}
