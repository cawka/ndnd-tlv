/**
 * @file ndndc-log.c
 * @brief Logging utilities for ndndc.
 *
...
ndndc_note(int lineno, const char *format, ...)
{
    ...
    if (verbose) {
        va_start(ap, format);
        gettimeofday(&t, NULL);
        fprintf(stderr, "%d.%06d ndndc[%d]:%d: ", (int)t.tv_sec, (unsigned)t.tv_usec, (int)getpid(), lineno);
...
on_error_exit(int res, int lineno, const char *msg)
{
    if (res >= 0)
        return;
    ndndc_fatal(lineno, "fatal error, res = %d, %s\n", res, msg);
}
