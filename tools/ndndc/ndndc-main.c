...
static int
read_configfile(struct ndndc_data *ndndc, const char *filename);
...
create_command_from_command_line(int argc, char **argv)
{
    ...
    if (out == 0) {
        ndndc_fatal(__LINE__, "Cannot allocate memory\n");
    }
...
main(int argc, char **argv)
{
    ...
    ndndc = ndndc_initialize_data();
    if (lifetime > 0)
        ndndc->lifetime = lifetime;
    if (optind < argc) {
        ...
        disp_res = ndndc_dispatch_cmd(ndndc, 0, argv[optind], cmd, argc - optind - 1);
        free(cmd);
        if (disp_res == INT_MIN) {
            usage(progname);
            goto Cleanup;
    ...
    if (configfile) {
        read_configfile(ndndc, configfile);
    }
    if (dynamic) {
        ndndc_daemonize(ndndc);
    }
    res = 0;
Cleanup:
    ndndc_destroy_data(&ndndc);
    return res;
}
    ...
    for (phase = 1; phase >= 0; --phase) {
        ...
        while (fgets((char *)buf, sizeof(buf), cfg)) {
            ...
            if (cmd == NULL) /* blank line */
                continue;
            res = ndndc_dispatch_cmd(ndndc, phase, cmd, rest_of_the_command, -1);
            retcode += res;
            if (phase == 1 && res < 0) {
